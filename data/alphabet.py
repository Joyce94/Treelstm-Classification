...
class Alphabet():
    def __init__(self, name, is_label=False, is_category=False, fix_flag=False):
        self.name = name
        self.is_label = is_label
        ...
        self.START = '<start>'
        self.PAD = '<pad>'
        if not self.is_label and not self.is_category: self.add(self.UNKNOWN)
        # if is_label: self.add(self.START)
        if not self.is_category: self.add(self.PAD)
        # print(self.word2id)
        # print(self.word2id[self.PAD])
