...
def patch_var(bucket, batch_length, target, target_start, target_end, params):
    # print(batch_length)
    if params.use_cuda:
        fea_var = Variable(torch.LongTensor(bucket[0])).cuda()
        label_var = Variable(torch.LongTensor(bucket[1])).cuda()
        pos_var = Variable(torch.LongTensor(bucket[2])).cuda()
        parse_var = Variable(torch.LongTensor(bucket[3])).cuda()
        rel_var = Variable(torch.LongTensor(bucket[4])).cuda()
        mask_var = Variable(torch.ByteTensor(bucket[-1])).cuda()
        length_var = Variable(torch.LongTensor(batch_length)).cuda()
        target_var = Variable(torch.LongTensor(target)).cuda()
        start = Variable(torch.LongTensor(target_start)).cuda()
        end = Variable(torch.LongTensor(target_end)).cuda()
    else:
        fea_var = Variable(torch.LongTensor(bucket[0]))
        label_var = Variable(torch.LongTensor(bucket[1]))
        pos_var = Variable(torch.LongTensor(bucket[2]))
        parse_var = Variable(torch.LongTensor(bucket[3]))
        rel_var = Variable(torch.LongTensor(bucket[4]))
        ...
        start = Variable(torch.LongTensor(target_start))
        end = Variable(torch.LongTensor(target_end))

    return fea_var, label_var, pos_var, parse_var, rel_var, mask_var, length_var, target_var, start, end
